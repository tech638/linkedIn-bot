...
function sheetCsvUrl() {
  const { GOOGLE_SHEET_ID, GOOGLE_SHEET_GID } = require("./hardcoded-config");
  ...
  const gid = process.env.GOOGLE_SHEET_GID || GOOGLE_SHEET_GID || "0";
...
async function fetchSheetCsv() {
  ...
  if (!url) {
    throw new Error(
      "Set GOOGLE_SHEET_ID (and optional GOOGLE_SHEET_GID) or GOOGLE_SHEET_CSV_URL in hardcoded-config.js"
    );
  }
...
async function readSheetFromApi() {
  ...
  if (!credPath || !sheetId) {
    throw new Error(
      "For private sheets set GOOGLE_APPLICATION_CREDENTIALS and GOOGLE_SHEET_ID in hardcoded-config.js"
    );
  }
