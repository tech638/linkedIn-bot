...
function assertBotProfileOnly() {
  ...
  if (process.env.CHROME_SYNC_PROFILE === "true") {
    throw new Error(
      "CHROME_SYNC_PROFILE is disabled — do not copy Profile 1 into the bot. Use npm run export-cookies."
    );
  }
}
...
async function prepareBotBrowser() {
  ...
  console.log(
    `Launching bot Chrome — LinkedIn only (no Profile 1 / Gmail)\n  ${CHROME_BOT_DATA_DIR} / ${BOT_PROFILE}`
  );
  if (headless) {
    console.log("  → Headless bot profile (session via linkedin-cookies.json or LINKEDIN_LI_AT).");
  }
