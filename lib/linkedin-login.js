...
async function tryCredentialsIfVisible(page) {
  ...
  if (!phase.hasLoginForm) {
    console.log("  → No email/password form (auto sign-in or checkpoint).");
...
/** Complete login when group visit lands on login / checkpoint. */
async function completeLoginIfNeeded(page, targetUrl) {
  const active = getActivePage(page);
  const phase = await readPagePhase(active);
  if (await isLoggedIn(page)) return { ok: true, alreadyLoggedIn: true };
  if (!phase.isLogin && !phase.isCheckpoint && !phase.autoSignIn) {
    return { ok: false, reason: "not_auth_page" };
  }
  console.log("  → Auth page detected — running login + 2Captcha flow…");
  await waitForAutoSignIn(page);
  await tryCredentialsIfVisible(page);
  return resolveLoginChallenges(page, targetUrl);
}
