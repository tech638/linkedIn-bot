...
async function waitAfterSignIn(page) {
  ...
  console.log(
    `  → Waiting for login (up to ${Math.round(LOGIN_MAX_WAIT_MS / 1000)}s)...`
  );
...
async function ensureLinkedInLoggedIn(page) {
  ...
  if (result.reason === "checkpoint") {
    console.warn("  → LinkedIn security checkpoint (2FA/captcha).");
  ...
  console.warn(
    `  → Login failed after ${Math.round(LOGIN_MAX_WAIT_MS / 1000)}s (url: ${url || "unknown"}).`
  );
