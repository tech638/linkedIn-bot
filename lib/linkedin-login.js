  ...
  const body = await page
    .evaluate(() => {
      ...
      const hasCaptcha = !!document.querySelector(
        'iframe[src*="captcha"], iframe[src*="recaptcha"], #captcha-internal'
      );
...
async function waitAfterSignIn(page) {
  ...
  console.log(
    `  → Waiting for session (up to ${Math.round(LOGIN_MAX_WAIT_MS / 1000)}s, progress every ${Math.round(LOGIN_PROGRESS_LOG_MS / 1000)}s)...`
  );

  while (Date.now() - started < LOGIN_MAX_WAIT_MS) {
    if (await hasSessionCookie(active)) {
    ...
    if (state.checkpoint || state.hasCaptcha || state.pinInput) {
      return { ok: false, reason: "checkpoint", url: state.url };
    }
    ...
    if (Date.now() - lastLog >= LOGIN_PROGRESS_LOG_MS) {
      ...
      console.log(`  → Login still pending (${elapsed}s) — ${shortUrl || "no url"}`);
      lastLog = Date.now();
    }
...
async function tryRestoreSavedSession(page) {
  const cookies = loadSessionCookies();
  if (!cookies.length) return { ok: false };

  const source = process.env.LINKEDIN_LI_AT
    ? "LINKEDIN_LI_AT env"
    : sessionFilePaths().find((p) => {
        try {
  ...
  console.log(`  → Restoring LinkedIn session (${source})...`);
  await restoreLinkedInSession(page, cookies);

  if (await isLoggedIn(page)) {
    markLoggedIn(page);
    console.log("  → Session restore OK.");
    return { ok: true, sessionRestore: true };
  }

  console.warn("  → Saved session invalid or expired — re-export cookies locally.");
  return { ok: false, reason: "session_expired" };
}

function printServerSessionHelp() {
  console.warn(
    "  → Railway cannot complete LinkedIn email/password login (stays on /login/)."
  );
  console.warn("  → Fix: on your laptop run `npm run export-cookies` after logging in, then:");
  console.warn("     • Railway → volume /app/data → upload linkedin-cookies.json");
  console.warn("     OR set Railway variable LINKEDIN_LI_AT=<value from DevTools → Application → Cookies → li_at>");
}
...
async function ensureLinkedInLoggedIn(page) {
  ...
  const restored = await tryRestoreSavedSession(page);
  if (restored.ok) return restored;

  const headlessServer =
    process.env.CHROME_HEADLESS === "true" &&
    process.env.SKIP_PASSWORD_LOGIN_ON_SERVER !== "false";

  if (headlessServer && !hasSessionSource()) {
    printServerSessionHelp();
    return { ok: false, reason: "no_session_on_server" };
  }

  if (headlessServer && restored.reason === "session_expired") {
    printServerSessionHelp();
    return { ok: false, reason: "session_expired" };
  }
  ...
  if (result.reason === "checkpoint") {
    console.warn(
      "  → LinkedIn security checkpoint (2FA/captcha). Log in once in bot Chrome locally, then copy the profile to Railway /app/data."
    );
  ...
  console.warn(
    `  → Login timed out after ${Math.round(LOGIN_MAX_WAIT_MS / 1000)}s (url: ${url || "unknown"}).`
  );
