...
async function tryRestoreSessionFromProfile(page) {
  ...
  for (const url of [FEED_URL, "https://www.linkedin.com/"]) {
    try {
...
async function ensureLinkedInLoggedIn(page) {
  ...
  if (
    (await readLiAtCookie(page)) &&
    !isLoginUrl(currentUrl) &&
    !/checkpoint|challenge|pin|verification/i.test(currentUrl)
  ) {
    markLoggedIn(page);
    return { ok: true, alreadyLoggedIn: true, sessionCookie: true };
  }
