...
async function detectCaptcha(page) {
  ...
  if (/arkoselabs|funcaptcha|fcapi/i.test(framesJoined) || dom.funcaptchaPublicKey) {
    ...
    /linkedin\.com\/login/i.test(pageUrl) &&
    (frameUrls.length > 2 ||
      dom.iframes?.length > 0 ||
      /google\.com|arkose|hcaptcha|recaptcha/i.test(framesJoined))
  ) {
    type = "linkedin_login_iframe";
    details.publickey =
      dom.funcaptchaPublicKey || extractPublicKeyFromFrames(frameUrls);
    details.sitekey =
      dom.recaptchaSitekey || extractSitekeyFromFrames(frameUrls, "recaptcha");
    confidence = "medium";
  } else if (
  ...
  let solvable = isSolvableBy2Captcha(type);
  if (type === "linkedin_login_iframe" && (details.publickey || details.sitekey)) {
    solvable = true;
  }
