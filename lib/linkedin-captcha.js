...
async function detectCaptcha(page) {
  ...
  const dom = await page
    .evaluate(() => {
      ...
      return {
        ...
        hasPin: !!document.querySelector(
          'input[name="pin"], input[autocomplete="one-time-code"]'
        ),
        hasPhoneVerify: /verify your phone|enter the code|verification code/i.test(lower),
        recaptchaSitekey:
  ...
  let type = "none";
  let confidence = "low";
  const details = { pageUrl, frameCount: frameUrls.length };
  ...
  if (dom.hasPin) {
    ...
    confidence = "high";
  } else if (/arkoselabs|funcaptcha|fcapi/i.test(framesJoined) || dom.funcaptchaPublicKey) {
  ...
  } else if (/recaptcha|google\.com\/recaptcha/i.test(framesJoined) || dom.recaptchaSitekey) {
    type = /recaptcha\/enterprise|enterprise/i.test(framesJoined)
      ? "recaptcha_enterprise"
      : "recaptcha_v2";
    details.sitekey = dom.recaptchaSitekey || extractSitekeyFromFrames(frameUrls, "recaptcha");
    confidence = details.sitekey ? "high" : "medium";
  ...
  return { present, type, confidence, details, dom, frameUrls: frameUrls.filter(Boolean) };
}
...
function logCaptchaDetection(info) {
  ...
  if (info.type === "linkedin_pin_2fa" || info.type === "linkedin_phone_verification") {
    console.log("  → 2Captcha cannot solve this — complete manually in bot profile.");
  }
  console.log("  ═══════════════════════");
}
...
async function solve2Captcha(info, pageUrl, page) {
  ...
  if (type === "recaptcha_v2" || type === "recaptcha_enterprise") {
    const googlekey = info.details.sitekey;
...
async function trySolveLoginCaptcha(page, clickSignInFn) {
  ...
  if (
    info.type === "linkedin_pin_2fa" ||
    info.type === "linkedin_phone_verification"
  ) {
  ...
  try {
    ...
    if (typeof clickSignInFn === "function") {
      console.log("  → Re-clicking sign in after captcha token…");
