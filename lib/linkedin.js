...
async function visitGroup(page, groupUrl) {
  ...
  const { completeLoginIfNeeded } = require("./linkedin-login");
  await completeLoginIfNeeded(page, groupUrl);
...
async function clickGroupPostButton(page, groupUrl, extraTriggers = []) {
  const { isLoggedIn, ensureLinkedInLoggedIn, logBotUrl } = require("./linkedin-login");

  if (!(await isLoggedIn(page))) {
    const login = await ensureLinkedInLoggedIn(page, { targetUrl: groupUrl });
    if (!login.ok) {
...
async function assessGroupPage(page) {
  ...
  const { isLoggedIn, logBotUrl } = require("./linkedin-login");
