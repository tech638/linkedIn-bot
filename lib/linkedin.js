...
async function visitGroup(page, groupUrl) {
  ...
  if (isLoginRedirectUrl(url) || !(await isLoggedIn(page))) {
    const cleared = await handleLinkedInAuthWall(page, { targetUrl: groupUrl });
    if (!cleared.ok) {
      console.warn(
        `  → Could not pass LinkedIn security check (${cleared.reason || "auth_wall"}).`
      );
    }
  }
...
async function prepareGroupPage(page, groupUrl) {
  ...
  const { isLoginRedirectUrl, handleLinkedInAuthWall } = require("./linkedin-login");
  if (isLoginRedirectUrl(url)) {
    await handleLinkedInAuthWall(page, { targetUrl: groupUrl });
    try {
      url = active.url();
    } catch {
      url = "";
...
async function clickGroupPostButton(page, groupUrl, extraTriggers = []) {
  ...
  if (!(await isLoggedIn(page))) {
    let postUrl = "";
    try {
      postUrl = getLinkedInPage(page).url();
    } catch {
      postUrl = "";
    }
    if (isLoginRedirectUrl(postUrl) && groupUrl) {
      const cleared = await handleLinkedInAuthWall(page, { targetUrl: groupUrl });
      if (!cleared.ok) {
        console.warn("  → LinkedIn security check blocked posting.");
...
async function assessGroupPage(page) {
  ...
  const { isLoggedIn, isLoginRedirectUrl } = require("./linkedin-login");
  if (isLoginRedirectUrl(url)) {
    return { accessible: false, reason: "auth_wall" };
  }
  if (!(await isLoggedIn(page))) {
